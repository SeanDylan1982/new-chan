...
export const BoardCard: React.FC<BoardCardProps> = ({
  ...
  const cardClass =
    viewMode === "list"
      ? "flex flex-row items-center bg-gray-50 border border-gray-200 rounded-lg px-6 py-4 hover:bg-white hover:border-cyan-300 hover:shadow-md transition-all duration-300 cursor-pointer w-full min-h-[80px] mb-2"
      : "bg-gray-50 border border-gray-200 rounded-xl p-6 hover:bg-white hover:border-cyan-300 hover:shadow-lg transition-all duration-300 cursor-pointer transform hover:scale-[1.02] group";
      ...
      {viewMode === "list" ? (
        <>
          <div className="flex-1 flex flex-col sm:flex-row sm:items-center gap-2">
            ...
            <div
              className={`hidden sm:inline-block px-3 py-1 rounded-full text-xs font-medium border ${getCategoryColor(
                board.category
              )}`}
            >
              {board.category}
          ...
          <div className="flex flex-col sm:flex-row items-end sm:items-center gap-2 ml-4 text-xs text-gray-500 min-w-[180px]">
            <span>{formatNumber(board.threadCount)} threads</span>
            <span>{formatNumber(board.postCount)} posts</span>
            <span className="text-gray-400">
              {formatTimeAgo(board.lastActivity)}
            </span>
            <span className="hidden sm:inline-block bg-gradient-to-r from-cyan-50 to-blue-50 border border-cyan-200 px-3 py-1 rounded-full text-cyan-600 font-medium">
              Enter Board →
            </span>
          </div>
        </>
